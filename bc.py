...
class BCNet2(nn.Module):
    """Simple class for non-linear bilinear connect network
    """

    def __init__(self, v_dim, q_dim, hid_num,act='ReLU', dropout=[.2, .5]):
        super(BCNet2, self).__init__()
        self.v_dim = v_dim
        self.q_dim = q_dim

        self.v_net = FCNet([v_dim, hid_num], act=act, dropout=dropout[0])
        self.q_net = FCNet([q_dim, hid_num], act=act, dropout=dropout[0])
        self.dropout = nn.Dropout(dropout[1])  # attention
