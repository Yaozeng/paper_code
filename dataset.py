...
def _load_dataset2(dataroot, name, img_id2val, label2ans):
    ...
    """
    question_path = os.path.join(
        "tools", 'coco_%s_question.json' % name)
    questions = sorted(json.load(open(question_path)), key=lambda x: x['question_id'])
    answer_path = os.path.join(dataroot, 'cache', 'coco_%s_target.pkl' % name)
    answers = cPickle.load(open(answer_path, 'rb'))
    answers = sorted(answers, key=lambda x: x['question_id'])
...
class COCOFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data', adaptive=False):
        super(COCOFeatureDataset, self).__init__()
        assert name in ['train', 'val', 'test-dev2015', 'test2015']

        ans2label_path = os.path.join(dataroot, 'cache', 'coco_ans2label.pkl')
        label2ans_path = os.path.join(dataroot, 'cache', 'coco_label2ans.pkl')
        self.ans2label = cPickle.load(open(ans2label_path, 'rb'))
        self.label2ans = cPickle.load(open(label2ans_path, 'rb'))
        self.num_ans_candidates = len(self.ans2label)
        ...
        self.entries = _load_dataset2(dataroot, name, self.img_id2idx, self.label2ans)
        self.tokenize()
        self.tensorize()
        self.v_dim = self.features.size(1 if self.adaptive else 2)
        self.s_dim = self.spatials.size(1 if self.adaptive else 2)
    ...
    def tensorize(self):
        ...
        for entry in self.entries:
            ...
            if None!=answer:
                labels = np.array([answer['labels']])
                scores = np.array([answer['types']], dtype=np.int64)
                if len(labels):
                    labels = torch.from_numpy(labels)
                    scores = torch.from_numpy(scores)
                    entry['answer']['labels'] = labels
    ...
    def __getitem__(self, index):
        ...
        if None!=answer:
            ...
            target = torch.zeros(self.num_ans_candidates)
            if labels is not None:
                target.scatter_(0, labels, 1)
            return features, spatials, question,target,question_id,image_id,types
        else:
            return features, spatials, question, question_id,image_id
